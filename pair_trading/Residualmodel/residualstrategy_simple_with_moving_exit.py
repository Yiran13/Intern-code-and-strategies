    ...
    def on_bars(self, bars: Dict[str, BarData]):
        ...
        if self.x_pos == 0 and self.y_pos == 0:
            ...
            if self.spread_value >= self.spread_long_profit_exit:
                ...
                self.last_long_trade_profit = True
                print(f'时间{bars[self.y_symbol].datetime}','进入盈利状态',f'待多{self.y_symbol} {self.y_fixed_size} 手 待空{self.x_symbol} {self.x_fixed_size} 手',f'价差{self.spread_value}')
            # 多平止损
            ...
            if self.spread_value <= self.spread_short_profit_exit:
                ...
                self.last_short_trade_profit = True
                print(f'时间{bars[self.y_symbol].datetime}','进入盈利状态',f'待空{self.y_symbol} {self.y_fixed_size} 手 待多{self.x_symbol} {self.x_fixed_size} 手',f'价差{self.spread_value}')
